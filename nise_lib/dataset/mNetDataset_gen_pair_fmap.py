...
class mNetDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        @log_time("GEN 2 fmap by mask")
        def gen_pair_fmap_by_maskRCNN(img_file_paths):
            inp = []
            scales = []
            ...
            with torch.no_grad():
                hai = self.conv_body(torch.cat(inp).cuda())
        ...
        # haiy = gen_pair_fmap_by_maskRCNN([prev_img_file_path, cur_img_file_path])
        start = time.time()
        ...
        num_true_samples = num_pos_to_sample + num_neg_to_sample
        ...
        for i, s in enumerate(all_samples):
            ...
            all_union_boxes.append(union_box)
            p_joints_hmap, c_joints_hmap = self._get_one_sample_joint_hmap(
                union_box, p_box_idx, c_box_idx,
                p_pred_joints, p_pred_joints_scores,
                c_pred_joints, c_pred_joints_scores
            )
            all_inputs[i, 512:527, :, :] = p_joints_hmap
            all_inputs[i, 527:542, :, :] = c_joints_hmap
        p_fmaps, c_fmaps = self._get_batch_fmaps(p_fmap_pkl, c_fmap_pkl, torch.cat(all_union_boxes))
        all_inputs[:num_true_samples, :256, :, :] = p_fmaps
        all_inputs[:num_true_samples, 256:512, :, :] = c_fmaps
    ...
    # @log_time('Get pair  joint map')
    def _get_one_sample_joint_hmap(self, u, p_box_idx, c_box_idx,
        ...
        return p_joints_hmap, c_joints_hmap
