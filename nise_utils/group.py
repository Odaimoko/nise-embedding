...
class Params:
    def __init__(self):
        ...
        self.partOrder = [i-1 for i in [1, 2, 3, 4, 5,
                                        6, 7, 12, 13, 8, 9, 10, 11, 14, 15, 16, 17]]
...
def match_by_tag(inp, params, pad=False):
    ...
    for i in range(params.num_parts):
        ...
        if i == 0 or len(dic) == 0:
            ...
            if params.ignore_too_much and len(actualTags) == params.max_num_people:
                continue
            diff = ((joints[:, None, 3:] - np.array(actualTags)
                     [None, :, :])**2).mean(axis=2) ** 0.5
            if diff.shape[0] == 0:
                continue
            ...
            if diff.shape[0] > diff.shape[1]:
                diff = np.concatenate(
                    (diff, np.zeros((diff.shape[0], diff.shape[0] - diff.shape[1])) + 1e10), axis=1)
    ...
    if pad:
        ...
        if num < params.max_num_people:
            padding = np.zeros(
                (params.max_num_people-num, params.num_parts, default_.shape[1]))
            if num > 0:
...
class HeatmapParser():
    ...
    def match(self, tag_k, loc_k, val_k):
        def match(x): return match_by_tag(x, self.param)
    ...
    def calc(self, det, tag):
        ...
        tag_k = torch.stack([torch.gather(tag[:, :, :, i], 2, ind)
                             for i in range(tag.size()[3])], dim=3)
